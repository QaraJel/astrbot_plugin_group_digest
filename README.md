...
实时采集 QQ 群聊文本消息（暂时仅支持文本，图像语音的配置参数选项未实装），定时调用 LLM 生成结构化摘要，以图片或 Markdown 文件形式输出到群聊。

## 基本功能

- **消息采集** — 监听群聊/私聊文本消息，流式追加到按时间命名的汇总文件（下文称原始文件），确保日志不会丢弃任何记录
